...
b =russianBuilding()
v = hold(P3(0,0,0), key("left-arrow", P3(-1.75, 0, 0)) + key("right-arrow", P3(1.75, 0, 0))+ key(" ", P3(0,0,0)))
b.position = P3(0,0,-2) + integral(v)


def shootBall(m, v):
    sphere(size = .1, position = now(b.position) + integral(P3(0,0,3)), duration = 2, collection = missles, color = (yellow))
...
def blowUp(m, v):
    ...
    play("explosion1.wav")
    resetWorld()
    text, ("Gurilla Warfare Game Over")
...
def randomPanda(m, v):
    ...
    b.react(hit(p, missles), destruct),fireWork(position = P3 (0,0,0))
